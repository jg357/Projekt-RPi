...
//Activity fuer die 7 Tage Temperatur Chart Activity
public class TempChart7d extends Activity implements SimpleGestureListener{
	...
	String[] tempChart = new String[24];
	int max;
	int min;
	...
	private String[] mMonth = new String[] {
	 ...
	 private void OpenChart()
	    {
	     ...
	     max=Integer.parseInt(tempChart[1]);	// Wandelt den String vom Array element 2 in eine Int um
	     min=Integer.parseInt(tempChart[1]);	// um diesen als Ausgangswert für einen Vergleich zu speichern, der in der 
												// Zeile 157 benutzt wird um die Kurve mittig vom Anzeigebereich zu platzieren
	     for(int i=1;i<=24;i++){
	    	x[i-1] =Integer.parseInt(tempChart[i]);
	    	if (max < x[i-1])
	    		max=x[i-1];
	    	if (min > x[i-1])
	    		min=x[i-1];
	     ...
	     XYSeries xSeries=new XYSeries("Temperatur");	// Legt den Namen für die Kurve fest.
	 ...
	 dataset.addSeries(xSeries);
	     ...
	     mRenderer.setChartTitle("Temperatur 7 Tage");
	     mRenderer.setXTitle("Stunden");
	     mRenderer.setYTitle("Thermo. °C");
	 ...
	 // Methode fuer den Swipe
	     public void onSwipe(int direction) {
	      ...
	      case SimpleGestureFilter.SWIPE_RIGHT : 
	    	  startActivity(new Intent(this,HumChart7d.class));
	    	  											 break;
	      case SimpleGestureFilter.SWIPE_LEFT :  
	    	  startActivity(new Intent(this,HumChart3d.class));	    	  
	    	  											break;
	      }
	      }
	 ...
	 class ClientThread implements Runnable {
			...
			public void run() {
				try {
					...
					out.println("TempChart7d!");	// Gibt den Pi den Befehl die Chart Werte zu uebertragen!
					out.flush();
				...
				try {
					while (i) {		// Solange die Activity aktiv ist werden mit dem BufferdReader
						...
						if (line != null) {
							Log.d("MyApp", line);						
						    if(line.contains("TempChart7d:")){	// Wenn die Nachricht TempChart7d vom Pi kommt so
						    	Log.d("MyApp", line);
						    	tempChart=line.split(" ");	// wird der String vom gesplittet und in das Array geschrieben
