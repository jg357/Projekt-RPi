...
public class WetterComActivity11Uhr extends Activity implements SimpleGestureListener {
	String line;
	Thread thread;
	TextView wetter_date_11_00;
	TextView wetter_vorhersage_11_00;
	TextView wetter_niederschlagswahrscheinlichkeit_11_00;
	TextView wetter_minimal_temp_11_00;
	TextView wetter_maximal_temp_11_00;
	TextView wetter_wind_geschwindigkeit_11_00;
	TextView ueberschrift_11_00;
	String zeitraum ="";
	...
	protected void onResume() {
	    ...
	    Log.d("MyApp", "onResume! Wetter11Uhr");	
		long endZeitpunkt = System.currentTimeMillis()+10000;
	    setContentView(R.layout.wetter_com_11_uhr);
		wetter_date_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_date_11_00);
		wetter_vorhersage_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_vorhersage_11_00);
		wetter_niederschlagswahrscheinlichkeit_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_niederschlagswahrscheinlichkeit_11_00);
		wetter_minimal_temp_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_minimal_temp_11_00);
		wetter_maximal_temp_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_maximal_temp_11_00);
		wetter_wind_geschwindigkeit_11_00 = (TextView) findViewById(R.id.wetter_com_txt_field_windgeschwindigkeit_11_00);
		ueberschrift_11_00 = (TextView) findViewById(R.id.wetter_willkommen_11_00);
		...
		if (zeitraum.contains("Zeitraum:")){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_date_11_00.setText(zeitraum);// dann gebe diese Daten als Textzeile auf der Activity aus
			i= true;
		}
		
		if(wetterprognose.contains("Wetterprognose:")){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_vorhersage_11_00.setText(wetterprognose);// dann gebe diese Daten als Textzeile auf der Activity aus
		}
		if(niederschlagswahr.contains("Niederschlagswahr.:")){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_niederschlagswahrscheinlichkeit_11_00.setText(niederschlagswahr);// dann gebe diese Daten als Textzeile auf der Activity aus
		}
		if(minimaltemp.contains("Minimaltemperatur:")){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_minimal_temp_11_00.setText(minimaltemp);// dann gebe diese Daten als Textzeile auf der Activity aus
		}
		if(maximaltemp.contains("Maximaltemperatur:")){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_maximal_temp_11_00.setText(maximaltemp);// dann gebe diese Daten als Textzeile auf der Activity aus
		}
		if(windgeschw.contains("Windgeschwindigkeit:")&& i==true){// Wenn der String der Website zerlegt wurde und fuer die Ausgabe formatiert
			wetter_wind_geschwindigkeit_11_00.setText(windgeschw);// dann gebe diese Daten als Textzeile auf der Activity aus
	...
	public void onButtonClick(View view) {// Button Listner Methode
		if (view.getId() == R.id.btn_wetter_06_00){// Oeffnet bei Buttonklick die Main Activity 
			startActivity(new Intent(this,WetterComActivity06Uhr.class));
		}
		if (view.getId() == R.id.btn_wetter_17_00){// Offnet die Vorhersage fuer den nachsten Zeitraum
	...
	class ClientThread implements Runnable {
		...
		public void run() {
			...
			anfang = wetter.indexOf("<time value=\"11:00\">");// sucht nach dem String Offset
			...
			pc_11_00 = (4 + (wetter.indexOf("<pc>", anfang)));
			for (int i = 0; i < 2; i++) {
				bearbeitet = wetter.charAt(pc_11_00 + i);
				if (bearbeitet != '<' && bearbeitet != '/')
				pc_11_00s += bearbeitet;
			}
			niederschlagswahr=("Niederschlagswahr.: " + pc_11_00s + "%");
			
			
			tn_11_00 = (4 + (wetter.indexOf("<tn>", anfang)));
			for (int i = 0; i < 2; i++) {
				bearbeitet = wetter.charAt(tn_11_00 + i);
				if (bearbeitet != '<' && bearbeitet != '/')
				tn_11_00s += bearbeitet;
			}
			minimaltemp=("Minimaltemperatur: " + tn_11_00s + " Grad");
			
			
			tx_11_00 = (4 + (wetter.indexOf("<tx>", anfang)));
			for (int i = 0; i < 2; i++) {
				bearbeitet = wetter.charAt(tx_11_00 + i);
				if (bearbeitet != '<' && bearbeitet != '/')
				tx_11_00s += bearbeitet;
			}
			maximaltemp=("Maximaltemperatur: " + tx_11_00s + " Grad");
			
			
			ws_11_00 = (4 + (wetter.indexOf("<ws>", anfang)));
			for (int i = 0; i < 3; i++) {
				bearbeitet = wetter.charAt(ws_11_00 + i);
				if (bearbeitet != '<' && bearbeitet != '/')
				ws_11_00s += bearbeitet;
			}
			windgeschw=("Windgeschwindigkeit: " + ws_11_00s + " km/h");
