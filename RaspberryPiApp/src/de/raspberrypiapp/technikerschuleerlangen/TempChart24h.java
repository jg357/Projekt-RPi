...
//Activity fuer die 24 Stunden Temperatur Chart Activity
public class TempChart24h extends Activity implements SimpleGestureListener{
	...
	private String[] mMonth = new String[] {
	 ...
	 private void OpenChart()
	    {
	     ...
	     min=Integer.parseInt(tempChart[1]);// um diesen als Ausgangswert für einen Vergleich zu speichern, der in der 
											// Zeile 159 benutzt wird um die Kurve mittig vom Anzeigebereich zu platzieren
	 ...
	 dataset.addSeries(xSeries);
	     ...
	     mRenderer.setChartTitle("Temperatur 24h");
	 ...
	 class ClientThread implements Runnable {
			...
			public void run() {
				try {
					...
					out.println("TempChart!");		// Gibt den Pi den Befehl die Chart Werte zu uebertragen!
					out.flush();
				...
				try {
					while (i) {		// Solange die Activity aktiv ist werden mit dem BufferdReader
						...
						if (line != null) {
							Log.d("MyApp", line);						
						    if(line.contains("TempChart:")){ // Wenn die Nachricht TempChart vom Pi kommt so
						    	Log.d("MyApp", line);
