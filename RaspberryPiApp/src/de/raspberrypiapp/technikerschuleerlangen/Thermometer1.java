...
public final class Thermometer1 extends View implements SensorEventListener {
	...
	public Thermometer1(Context context) {
		super(context);
		init();
	}

	public Thermometer1(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public Thermometer1(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init() {
		...
		long endZeitpunkt = System.currentTimeMillis()+10000; // Setze die Deathtime auf 10 Sekunden
	     while(Thermometer.feuchteWert == -10){ // Solange der Feuchte wert noch auf den defaultwert -10 steht fuehre die
	    	 try {								// Endlosschleife aus!
				Thread.sleep(100);
		...
		setHandTarget(Thermometer.feuchteWert); // Wenn der Feuchte Wert vom Pi zurueckgekommen ist so setze diesen!
		str = ("Luftfeuchtigkeit: "+Thermometer.feuchteWert+"%"); // Beschriftung fuer das Analoge Instrument
