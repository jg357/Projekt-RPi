...
public final class Thermometer extends View implements SensorEventListener {
	...
	static int tempWert=-10;
	static 	int feuchteWert=-10;
	...
	private void initDrawingTools() {
		...
		scalePaint.setLinearText(true);// hinzugefügt
	...
	class ClientThread implements Runnable {
		...
		public void run() {
			try {
				...
				out.println("ZimmerauswertungAnalog"); // Gib den Pi den Befehl die Werte für das Anzeigeinstrument zu uebertragen
				out.flush();
			...
			try {
				while (i) {  // Solange die Activity aktiv ist werden mit dem BufferdReader
					...
					if (line != null) {
						Log.d("MyApp", line);						
					    if(line.contains("ZimmerauswertungTemp:")){ // Wenn die Werte fuer die Temperatur kommen dann..
						werte[1]=line.substring(21,23); // Zerlege den String und schreibe ihn in das Array fuer die Werte
						tempWert = Integer.parseInt(werte[1]); // und wandele Ihn in eine Integer um 
					    Log.d("MyApp", line);
					    Log.d("MyApp", werte[1]);
					    }
					    if(line.contains("ZimmerauswertungFeuchte:")){	// Wenn die Werte fuer die Luftfeuchtigkeit kommen dann..
							werte[0]=line.substring(24,26);				// Zerlege den String und schreibe ihn in das Array fuer die Werte
							feuchteWert = Integer.parseInt(werte[0]);	// und wandele Ihn in eine Integer um 
						    Log.d("MyApp", werte[0]);
						    Log.d("MyApp", "Pause!");	
						    try{
							    out.println("BYE!");	// Geb den Pi die Rückmeldung das es die Verbindung beenden kann
							    out.flush();
							    }
							    catch(NullPointerException e){
							    	Log.e("MyApp", "Out Writer nicht bereit!");
							    }
						    i=false; // Boolean fuer diese Endlosschleife wird zurueckgesetzt
						    }
			...
			setHandTarget(tempWert); // Hier wird der Temperaturwert dem Analogen Anzeigeinstrument uebergeben
			str = ("Temperatur: "+tempWert+"°C"); // Hier wird der String zusaammengesetzt der spaeter auf der Beschriftung
		}										  // des Analogen Instruments hinzugefuegt wird.
